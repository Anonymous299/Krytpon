...
public class Inbox extends JFrame implements ActionListener, MouseListener
{
    ...
    public void actionPerformed(ActionEvent eve) {
       if(eve.getSource()==btn2 )
       {
          if(delete==false)
          {
           delete = true;
          btn2.setText("Delete 0");
        }
        else
        {
          delete = false;
          ...
          {
            button.get(Integer.parseInt(deleteList.get(i))).setFocusPainted(false);
            button.get(Integer.parseInt(deleteList.get(i))).setContentAreaFilled(false);
            }
            out.println("d");//karna hai
            for(int i=0;i<deleteList.size();i++)
            {
              out.println(Integer.parseInt(deleteList.get(i)));
            }
            out.println("stopitrightnow!@#$");
            try
            {
            Inbox i = new Inbox(username);
        }
        catch(Exception ex){}
            frame.dispose();
        }
       ...
       int i=0;
        ...
        {
          if(deleteList.get(i).equals(Integer.toString(num)))
          {
              isThere = true;
              break;
            }
        ...
        if(!isThere)
        {
        deleteList.add(Integer.toString(num));
        btn2.setText("Delete " + deleteList.size() );
        button.get(num).setFocusPainted(true);
            button.get(num).setContentAreaFilled(true);
    }
    else
    {
      deleteList.remove(i);
      btn2.setText("Delete " + deleteList.size() );
      button.get(num).setFocusPainted(false);
            button.get(num).setContentAreaFilled(false);
